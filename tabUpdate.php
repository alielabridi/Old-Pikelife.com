<?php
    ...
    if($newUpdate = $newUpdate_query->fetch()){ 
        if($newUpdate["new_chat"] > 0){
            $hint = $hint = "<h3 class='widget_title'>Friends<span style='background-color:red; padding:1px 3px 1px 3px; margin-left:4px; color: white;border-radius: 10px;border-color: rgb(10, 10, 10);box-shadow: 2px 2px 2px #888888;'>". $newUpdate["new_chat"] ."</span></a></h3>";
        }else{
